...
const TeacherTaskLISt = () => {
  ...
  const statusDone = (id) => {
    axios
      .put(`http://localhost:8081/teacher-task/TeacherTask/status/${id}`, {
        status: "done",
        id: id,
      })
      .then((response) => {
              ...
              <tbody>
                {search(taskList).map((c, i) => (
                  <tr key={i} className="align-top">
                    ...
                    {c.status === "not done" && (
                      <td>
                        <span className="badge bg-warning">{c.status}</span>
                      </td>
                    )}
                    <td>
                      <button
                        className="btn btn-outline-success mr-5"
                        onClick={() => statusDone(c._id)}
                      >
